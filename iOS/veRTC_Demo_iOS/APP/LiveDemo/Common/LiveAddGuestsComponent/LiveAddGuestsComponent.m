...
- (NSString *)removeAddGuestsUid:(NSString *)uid userList:(NSArray<LiveUserModel *> *)userList {
    ...
    if (_liveAddGuestsRoomView && _userList.count > 0) {
        ...
        if (_userList.count > 0) {
            ...
            // When the current Lianmai user is greater than 0, only update the Lianmai list
...
- (void)clickApplyAction:(NSString *)hostID {
    ...
    [LiveRTMManager liveAudienceLinkmicApply:self.roomInfoModel.roomID
                                              block:^(NSString *linkerID,
                                                      RTMACKModel *model) {
        if (model.result ||
            model.code == RTMStatusCodeUserIsInviting ||
            model.code == RTMStatusCodeUserIsNewInviting) {
            // Initiate an invitation
            [[ToastComponent shareToastComponent] showWithMessage:[NSString stringWithFormat:@"您已向主播发起连麦申请，等待主播应答。"]];
...
- (void)liveAddGuestsListsView:(LiveAddGuestsListsView *)liveAddGuestsListsView
    ...
    [LiveRTMManager liveAudienceLinkmicInvite:self.roomInfoModel.roomID
        ...
        if (model.result ||
            ...
            [[ToastComponent shareToastComponent] showWithMessage:[NSString stringWithFormat:@"已发出邀请，等待对方应答"]];
        } else {
            [[ToastComponent shareToastComponent] showWithMessage:model.message];
        }
    }];
